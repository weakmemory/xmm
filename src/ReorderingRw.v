...
Lemma simrel_a_lab_wr
    ...
    (CORR : reord_step_pred)
    (SIMREL : reord_simrel) :
  eq a_t ∩₁ (acts_set G_s) ⊆₁ (R G_s ∪₁ W G_s).
Proof using.
  destruct SIMREL. destruct CORR.
  intros x COND. destruct COND as [EQ IN]; subst x.
  apply rsr_acts0 in IN. unfold extra_a in IN. desf.
  { destruct IN as [IN | IN]; subst.
    { destruct a as (NO & DA).
      destruct rsr_b_t_is_r_or_w0 with b_t; vauto.
      { right. unfold is_w in *.
        rewrite <- rsr_lab0 in H; vauto.
        unfold compose in H.
        arewrite (lab_s a_t = lab_s b_s); vauto.
        rewrite rsr_map_bt; vauto. }
      left. unfold is_r in *.
      rewrite <- rsr_lab0 in H; vauto.
      unfold compose in H.
      arewrite (lab_s a_t = lab_s b_s); vauto.
      rewrite rsr_map_bt; vauto. }
    exfalso. apply rsr_at_neq_bt0; vauto. }
  destruct IN as [IN | IN]; [ |basic_solver].
  apply not_and_or in n.
  destruct n as [DA | NO].
  apply NNPP in DA.
  { destruct rsr_b_t_is_r_or_w0 with b_t; [basic_solver | |].
    { right. unfold is_w in *.
      apply rsr_at_bt_ord0 in DA.
      rewrite <- rsr_lab0 in H; vauto.
      unfold compose in H.
      arewrite (lab_s a_t = lab_s b_s); vauto.
      rewrite rsr_map_bt; vauto. }
...
Proof using.
  ...
  { destruct IN as [IN | IN]; subst.
    { destruct a as (NO & DA).
      destruct rsr_a_t_is_r_or_w0 with a_t; vauto. }
    destruct a as (NO & DA).
    destruct rsr_b_t_is_r_or_w0 with b_t; vauto. }
  destruct IN as [IN | IN]; [ |basic_solver].
  apply not_and_or in n.
  destruct n as [DA | NO].
  apply NNPP in DA.
  { destruct rsr_a_t_is_r_or_w0 with a_t; [basic_solver | |].
    { right. unfold is_w in *.
      apply rsr_at_bt_ord0 in DA.
      rewrite <- rsr_lab0 in H; vauto.
      unfold compose in H.
      rewrite <- rsr_map_at; vauto. }
    left. unfold is_r in *.
    apply rsr_at_bt_ord0 in DA.
    rewrite <- rsr_lab0 in H; vauto.
    unfold compose in H.
    rewrite <- rsr_map_at; vauto. }
  apply rsr_at_bt_ord0 in INB.
  clear - NO INB. desf.
Qed.

Lemma simrel_a_lab_notrel
    (INB : E_t b_t)
    (CORR : reord_step_pred)
    (SIMREL : reord_simrel) :
  eq a_t ∩₁ (acts_set G_s) ⊆₁ set_compl (Rel G_s).
Proof using.
  destruct SIMREL. destruct CORR.
  intros x COND. destruct COND as [EQ IN]; subst x.
  apply rsr_acts0 in IN. unfold extra_a in IN. desf.
  { destruct IN as [IN | IN]; subst.
    { destruct a as (NO & DA).
      unfold set_compl. intros FALSE.
      destruct rsr_bt_nrel0 with b_t; vauto.
      unfold is_rel in *.
      unfold mod in *.
      rewrite <- rsr_map_bt in FALSE; vauto.
      rewrite <- rsr_lab0; vauto. }
    exfalso. apply rsr_at_neq_bt0; vauto. }
  destruct IN as [IN | IN]; [ |basic_solver].
  apply not_and_or in n.
  destruct n as [DA | NO].
  { apply NNPP in DA.
    unfold set_compl. intros FALSE.
    destruct rsr_bt_nrel0 with b_t; [basic_solver |].
    unfold is_rel in *.
    unfold mod in *.
    rewrite <- rsr_map_bt in FALSE; vauto.
    rewrite <- rsr_lab0; vauto. }
...
Lemma simrel_b_lab_notacq
    (INB : E_t a_t)
    (CORR : reord_step_pred)
    (SIMREL : reord_simrel) :
  eq b_t ∩₁ (acts_set G_s) ⊆₁ set_compl (Acq G_s).
Proof using.
  destruct SIMREL. destruct CORR.
  intros x COND. destruct COND as [EQ IN]; subst x.
  apply rsr_acts0 in IN. unfold extra_a in IN. desf.
  { destruct IN as [IN | IN]; subst.
    { destruct a as (NO & DA).
      unfold set_compl. intros FALSE.
      destruct rsr_at_nacq0 with a_t; vauto. }
    destruct a as (NO & DA). desf. }
  destruct IN as [IN | IN]; [ |basic_solver].
  apply not_and_or in n.
  destruct n as [DA | NO].
  { apply NNPP in DA.
    unfold set_compl. intros FALSE.
    destruct rsr_at_nacq0 with a_t; [basic_solver |].
    unfold is_acq in *.
    unfold mod in *.
    rewrite <- rsr_map_at in FALSE; vauto.
    rewrite <- rsr_lab0; vauto. }
  apply rsr_at_bt_ord0 in INB.
  clear - NO INB. desf.
Qed.
...
Proof using INV INV'.
  ...
  { destruct SIMREL'. rewrite rsr_co0.
    rewrite EXTRA; clear; basic_solver 8. }
  { destruct SIMREL'. rewrite rsr_rmw0.
    basic_solver. }
  { admit. }
  admit. (*TODO : add*) 
Admitted.
...
Lemma wf_rpo_immE G
    (WF : Wf G) :
  rpo_imm G ≡ ⦗acts_set G⦘ ⨾ rpo_imm G ⨾ ⦗acts_set G⦘.
Proof using.
  split; [| basic_solver].
  ...
  { constructor; ins.
    ...
    { split; [| basic_solver].
      ...
      { split; [| basic_solver].
        ...
        rewrite NEWSB. rewrite rsr_sb0. unfold swap_rel.
        arewrite (eq b_t ∩₁ E_t ≡₁ ∅).
        { split; basic_solver. }
    ...
    { apply Consistency.read_extent with (G_t := G_t)
      ...
      { unfold G_s'' at 2; ins.
        ...
        { unfold sb at 1. ins. rewrite NEWSB.
          rewrite seq_union_l. destruct SIMREL.
          unfold WCore.sb_delta. apply inclusion_union_l.
          { rewrite rsr_sb0. arewrite_id ⦗E_s⦘; rels.
            apply inclusion_union_l.
            { apply inclusion_union_l.
      ...
      { split; [| basic_solver]. unfold sb at 1.
        ...
        destruct SIMREL. rewrite rsr_sb0.
        unfold swap_rel.
        arewrite (eq b_t ∩₁ E_t ≡₁ ∅).
        { split; basic_solver. }
        rels. unfold extra_a. desf.
        ...
        { arewrite (⦗eq b_t⦘ ⨾ mapper ↑ (⦗E_t⦘ ⨾ sb_t ⨾ ⦗E_t⦘) ∩ same_loc (upd lab_s b_t (Aload ex0 o0 l0 v)) ⊆
          ...
          rewrite rsr_codom0. basic_solver 12. }
      ...
      { unfold sb at 1. ins. rewrite NEWSB.
        ...
        { intros x y COND. destruct COND as (z & (SB & SL) & INE).
          ...
          { intros FALSO.
            ...
            destruct NOTIN. destruct rsr_acts0.
            apply H0. left. vauto. }
          intros FALSO.
          destruct SB as (x0 & (C1 & EQ) & C2).
          rewrite <- FALSO in NOTIN.
...
Proof using.
  ...
  { assert (AINRW : eq a_t ∩₁ (acts_set G_s') ⊆₁ (R G_s' ∪₁ W G_s')).
    ...
    { unfold rpo_imm. rewrite !seq_union_r.
      ...
      rels.
      unfold is_f; ins.
      clear - ANOTB. unfolder.
      intros x y ((COND1 & EQ) & (COND2 & COND3)).
      subst. rewrite updo in COND1; vauto.
      rewrite upds in COND1. desf. }
        ...
        { unfold rpo. transitivity ((⦗E_s⦘ ⨾ rpo_imm G_s' ⨾ ⦗E_s ∪₁ eq a_t⦘)⁺).
          ...
          { unfold rpo_imm. rewrite !seq_union_l. rewrite !seqA.
            ...
            { intros x y COND. destruct COND as (EQ & COND); subst y.
              ...
              { split; vauto.
                destruct SIMREL'.
                unfold is_f. destruct rsr_lab1 with x'; vauto.
                unfold compose. rewrite MAP; vauto. }
              unfold is_acq. unfold mod. destruct SIMREL'.
              destruct rsr_lab1 with x'; vauto.
              unfold compose. unfold is_acq in COND2. unfold mod in COND2.
              rewrite MAP; vauto. }
            arewrite (⦗Acq G_s' ∩₁ mapper ↑₁ E_t⦘
                        ⊆ mapper' ↑ ⦗Acq G_t' ∩₁ E_t'⦘).
            ...
            { intros x y COND. destruct COND as (EQ & COND); subst y.
              ...
              split; vauto.
              destruct SIMREL'.
              unfold is_rel. unfold mod. destruct rsr_lab1 with x'; vauto.
            ...
            { intros x y COND. destruct COND as (EQ & COND); subst y.
              ...
              { split; vauto.
                destruct SIMREL'.
                unfold is_w. destruct rsr_lab1 with x'; vauto.
                unfold compose. unfold is_w in COND1.
                rewrite MAP; vauto. }
              destruct SIMREL'.
              unfold is_rlx. unfold mod. destruct rsr_lab1 with x'; vauto.
              unfold compose. unfold is_rlx in COND2. unfold mod in COND2.
              rewrite MAP; vauto. }
            rewrite !collect_rel_union.
            apply union_more.
    ...
    { destruct SIMREL'.
      arewrite (G_s' = WCore.G X_s').
      rewrite rsr_sb0. rewrite !NEWEXA.
      arewrite (eq a_t ∩₁ E_t' ≡₁ ∅).
      { clear - ANOTIN'. basic_solver. }
      arewrite (eq b_t ∩₁ E_t' ≡₁ eq b_t).
      { split; [basic_solver |].
    ...
    { destruct SIMREL'.
      arewrite (G_s' = WCore.G X_s').
      rewrite rsr_rf0. apply union_more; vauto.
      apply seq_more; vauto.
      rewrite NEWEXA. unfold is_r.
    ...
    { destruct SIMREL'.
      arewrite (G_s' = WCore.G X_s').
      rewrite rsr_co0. unfold add_max.
      rewrite NEWEXA.
      ...
      unfold is_w. arewrite (WCore.G X_s' = G_s').
      unfold G_s'; ins. unfold upd.
      split; vauto. unfold set_inter.
      red; intros x COND. 
      destruct COND as [COND1 COND2].
      rewrite COND1 in COND2. desf.
  ...
  { apply Consistency.write_extent with (G_t := G_t')
    ...
    { destruct SIMREL'.
      arewrite (G_s' = WCore.G X_s').
      rewrite rsr_rf0. rewrite NEWEXA.
      arewrite (eq b_t ∩₁ R (WCore.G X_s') ≡₁ ∅).
      { split; [| basic_solver].
        unfold is_r. unfold G_s'; ins. unfold upd.
        red. intros x (EQ & COND).
        rewrite EQ in COND.
        assert (NEQA : x <> a_t).
        { intros FALSO. subst x. destruct ANOTINS; vauto. }
        basic_solver 42. }
      basic_solver 8. }
    { destruct SIMREL'.
      arewrite (G_s' = WCore.G X_s').
      ...
      { split; [basic_solver |].
        intros x XIN. unfold is_w. unfold G_s' in XIN.
        red. split; vauto.
        arewrite (WCore.G X_s' = G_s').
        unfold G_s'; ins. unfold upd.
        rewrite XIN.
      ...
      { unfold rpo_imm.
        ...
        { arewrite (G_s' = WCore.G X_s'). rewrite rsr_acts0.
          ...
          { destruct C1. subst x1.
            ...
            split; vauto. apply rsr_acts0; vauto. }
