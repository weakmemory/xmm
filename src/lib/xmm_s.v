...
From imm Require Import imm_s_hb_hb.
From imm Require Import imm.
...
Notation "'hb'" := (xmm_s_hb.hb G).
Notation "'sw'" := (xmm_s_hb.sw G).
...
Proof using WF.
  ...
  unfold sw, release, rs.
  rewrite rt_rf_rmw. arewrite (⦗Rlx⦘ ⨾ ⦗W⦘ ⊆ ⦗W⦘) by basic_solver.
  arewrite (⦗W⦘ ⨾ (rfi ⨾ rmw)＊ ⊆ ⦗W⦘ ⨾ (sb ∩ same_loc)^? ⨾ ⦗W⦘).
...
Proof using WF.
  ...
  { unfold fwbob.
    ...
    mode_solver. }
  unfold release.
  arewrite (⦗W ∩₁ Rel⦘ ⨾ sb^? ∩ (⦗Rel⦘ ⨾ (⦗F⦘ ⨾ sb)^? ⨾ ⦗Rlx⦘ ⨾ rs G) ⊆
            ⦗W ∩₁ Rel⦘ ⨾ sb^? ∩ (⦗Rel⦘ ⨾ rs G)).
  { type_solver 10. }
