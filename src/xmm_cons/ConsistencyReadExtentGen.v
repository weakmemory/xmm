...
Hypothesis (CONS : WCore.is_cons G_t sc_t).
...
Proof using INJ E_MAP IS_R IS_ACQ NIN CODOM_RPO RPO_MAP
  ...
  { case_refl _.
      { rewrite hb_helper; eauto. rewrite irreflexive_union. split.
        ...
        rewrite <- EQA in EQ. destruct read_rhb_codom as (IN1 & IN2); eauto.
      ...
      { rotate 1. eapply empty_irr.
        ...
        { apply T; basic_solver. }
        destruct Q. }
      assert (IN' : rhb_s ⨾ (drf ⨾ ⦗eq a⦘)⁻¹ ⨾ co_s ⨾ rf_s^? ⊆ rhb_s ⨾ (drf ⨾ ⦗eq a⦘)⁻¹ ⨾ co_s ⨾ ⦗W_s⦘ ⨾ rf_s^? ).
