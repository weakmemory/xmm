...
Proof using CTX.
  admit. (* True by monotonicity *)
Admitted.

Definition A_s' := extra_a X_t' a_t' b_t' a_s'.
Definition E_s' := mapper' ↑₁ E_t' ∪₁ A_s'.
Definition sb_s' := ⦗E_s'⦘ ⨾ ext_sb ⨾ ⦗E_s'⦘.

(* 'fake' srf. we later show that it is equal to srf ; [a_s'] *)
Definition srf_s'' :=
  (
    vf_s ⨾ sb_s' ⨾ ⦗A_s'⦘ ∩
    (fun x _ => loc_s x = WCore.lab_loc l_a')
  ) ⨾ ⦗WCore.lab_is_r l_a'⦘
    \ (co_s ⨾ vf_s ⨾ sb_s' ⨾ ⦗A_s'⦘).
...
Definition G_s' : execution := {|
  ...
  lab := lab_t' ∘ mapper';
    ...
    co := mapper' ↑ co_t' ∪
          add_max
            (extra_co_D (mapper' ↑₁ E_t') lab_s (WCore.lab_loc l_a'))
...
Lemma final_graph_with_srf_wf
    (INB : E_t' b_t')
    (NINA : ~ E_t' a_t') :
  Wf G_s''.
Proof using CTX.
  admit. (* We added an edge and a_s to intermediate graph *)
Admitted.

Lemma final_graph_with_srf_cons
    (INB : E_t' b_t')
    (NINA : ~ E_t' a_t') :
  irreflexive (hb G_s'' ⨾ (eco G_s'')^?).
Proof using CTX.
  admit. (* TODO (ask Evgenii) *)
Admitted.

(* Srf goodness *)
Lemma final_graph_with_srf_srf :
  (rf G_s'') ⨾ ⦗A_s'⦘ ≡ (srf G_s'') ⨾ ⦗A_s'⦘.
Proof using CTX.
  admit.
Admitted.

Lemma final_graph_simrel :
